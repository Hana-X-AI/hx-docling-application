...
| `X-RateLimit-*` | Yes | Legacy rate limiting headers (Reset = epoch seconds) |
| `RateLimit-*` | Yes | RFC 9238 rate limiting headers (Reset = delta seconds) |
| `Retry-After` | On 429 | Seconds to wait before retrying (delta seconds) |
 ...
 *
 * @param result - The rate limit check result containing remaining count and retry delay
 * @returns Headers object with both X-RateLimit-* (epoch) and RateLimit-* (delta) headers
 */
export function buildRateLimitHeaders(result: RateLimitResult): Record<string, string> {
  const { remaining, retryAfter = 0 } = result;
  const nowEpochSeconds = Math.floor(Date.now() / 1000);
  const resetEpochSeconds = nowEpochSeconds + retryAfter;

  return {
    // Legacy headers (X-RateLimit-*): Reset uses epoch seconds (Unix timestamp)
    'X-RateLimit-Limit': String(RATE_LIMIT_MAX),
    'X-RateLimit-Remaining': String(remaining),
    'X-RateLimit-Reset': String(resetEpochSeconds),  // Epoch: e.g., 1702483260

    // RFC 9238 headers (RateLimit-*): Reset uses delta seconds (time until reset)
    'RateLimit-Limit': String(RATE_LIMIT_MAX),
    'RateLimit-Remaining': String(remaining),
    'RateLimit-Reset': String(retryAfter),           // Delta: e.g., 45

    // RFC 9110 Retry-After: Uses delta seconds (time to wait before retrying)
    'Retry-After': String(retryAfter),               // Delta: e.g., 45
  };
}

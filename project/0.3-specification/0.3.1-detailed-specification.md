...
| `X-Request-ID` | Always | Echo client ID or generate UUID |
| `X-RateLimit-Limit` | On rate-limited endpoints | Maximum requests per window |
| `X-RateLimit-Remaining` | On rate-limited endpoints | Requests remaining in window |
| `X-RateLimit-Reset` | On 429 only | Epoch seconds when window resets |
| `RateLimit-Limit` | On rate-limited endpoints | RFC 9238: Maximum requests per window |
| `RateLimit-Remaining` | On rate-limited endpoints | RFC 9238: Requests remaining in window |
| `RateLimit-Reset` | On 429 only | RFC 9238: Delta seconds until window resets |
| `Retry-After` | On 429 only | Seconds to wait before retrying (delta seconds) |
...
if count < limit then
  ...
  -- Get oldest entry to calculate retry time (delta seconds until window slides)
  local oldest = redis.call('ZRANGE', key, 0, 0, 'WITHSCORES')
  local retryAfter = oldest[2] and (window - (now - oldest[2])) or window
  return {0, 0, math.ceil(retryAfter)}  -- denied, remaining, retryAfter (delta seconds)
end
`;
...
export interface RateLimitResult {
  ...
  remaining: number;
  /**
   * Seconds until the rate limit window resets (delta seconds, not epoch).
   * Used directly for RFC 9238 RateLimit-Reset and Retry-After headers.
   * For legacy X-RateLimit-Reset (epoch), add to current timestamp.
   */
  retryAfter?: number;
}
...
/**
 * In-memory rate limit check for single-instance fallback.
 * Returns retryAfter in delta seconds (time until window resets).
 */
function checkInMemoryRateLimit(sessionId: string): RateLimitResult {
  ...
  if (entry.count >= RATE_LIMIT_MAX) {
    // Calculate delta seconds until window resets (not epoch timestamp)
    const deltaSecondsUntilReset = RATE_LIMIT_WINDOW_SECONDS - (now - entry.windowStart);
    return {
      allowed: false,
      remaining: 0,
      retryAfter: deltaSecondsUntilReset,  // Delta seconds, e.g., 45
    };
  }
...
Two header families are provided for maximum client compatibility:

#### Legacy Headers (X-RateLimit-*)

These headers follow the draft convention and use **Unix epoch timestamps** for the Reset value.

| Header | Value Type | Description |
|--------|-----------|-------------|
| `X-RateLimit-Limit` | Integer | Maximum requests allowed per window (e.g., `10`) |
| `X-RateLimit-Remaining` | Integer | Requests remaining in current window (e.g., `7`) |
| `X-RateLimit-Reset` | **Epoch seconds** | Unix timestamp when the rate limit window resets (e.g., `1702483260`) |

#### Standard Headers (RFC 9238)

These headers follow [RFC 9238](https://www.rfc-editor.org/rfc/rfc9238.html) and use **delta seconds** for the Reset value.

| Header | Value Type | Description |
|--------|-----------|-------------|
| `RateLimit-Limit` | Integer | Maximum requests allowed per window (e.g., `10`) |
| `RateLimit-Remaining` | Integer | Requests remaining in current window (e.g., `7`) |
| `RateLimit-Reset` | **Delta seconds** | Seconds until the rate limit window resets (e.g., `45`) |

#### Retry-After Header (RFC 9110)

| Header | Value Type | Description |
|--------|-----------|-------------|
| `Retry-After` | **Delta seconds** | Seconds the client should wait before retrying (e.g., `45`) |
...
/**
 * Builds rate limit response headers for both legacy and RFC 9238 formats.
 *
 * - Limit and Remaining headers are always returned for rate-limited endpoints
 * - Reset and Retry-After headers are ONLY returned on 429 responses (when retryAfter is present)
 *
 * @param result - The rate limit check result containing remaining count and optional retry delay
 * @returns Headers object with rate limit headers (Reset/Retry-After only when rate limited)
 */
export function buildRateLimitHeaders(result: RateLimitResult): Record<string, string> {
  const { remaining, retryAfter } = result;

  // Base headers: always returned for rate-limited endpoints
  const headers: Record<string, string> = {
    // Legacy headers (X-RateLimit-*)
    'X-RateLimit-Limit': String(RATE_LIMIT_MAX),
    'X-RateLimit-Remaining': String(remaining),

    // RFC 9238 headers (RateLimit-*)
    'RateLimit-Limit': String(RATE_LIMIT_MAX),
    'RateLimit-Remaining': String(remaining),
  };

  // Reset and Retry-After headers: ONLY returned on 429 (when rate limited)
  // Do NOT default retryAfter to 0 - it must be explicitly set when rate limiting
  if (retryAfter !== undefined && retryAfter > 0) {
    const nowEpochSeconds = Math.floor(Date.now() / 1000);
    const resetEpochSeconds = nowEpochSeconds + retryAfter;

    // Legacy: Reset uses epoch seconds (Unix timestamp)
    headers['X-RateLimit-Reset'] = String(resetEpochSeconds);

    // RFC 9238: Reset uses delta seconds (time until reset)
    headers['RateLimit-Reset'] = String(retryAfter);

    // RFC 9110 Retry-After: delta seconds (time to wait before retrying)
...
// Example values when ALLOWED (remaining=5, retryAfter=undefined):
// {
//   'X-RateLimit-Limit': '10',
//   'X-RateLimit-Remaining': '5',
//   'RateLimit-Limit': '10',
//   'RateLimit-Remaining': '5',
//   // Note: No Reset or Retry-After headers on allowed requests
// }

// Example values when RATE LIMITED (remaining=0, retryAfter=45):
// {
//   'X-RateLimit-Limit': '10',
//   'X-RateLimit-Remaining': '0',
//   'X-RateLimit-Reset': '1702483260',    // Epoch timestamp when window resets
//   'RateLimit-Limit': '10',
//   'RateLimit-Remaining': '0',
//   'RateLimit-Reset': '45',              // 45 seconds until reset
...
describe('Security: Rate Limiting', () => {
  ...
  it('returns correct rate limit header semantics', async () => {
    const sessionId = 'test-headers-' + Date.now();
    ...
    // Verify legacy headers (X-RateLimit-*) use epoch timestamps
    const xResetEpoch = parseInt(response.headers.get('X-RateLimit-Reset') || '0', 10);
    expect(xResetEpoch).toBeGreaterThan(beforeRequest);  // Must be epoch timestamp
    expect(xResetEpoch).toBeLessThanOrEqual(beforeRequest + 60 + 5);  // Within window + buffer

    // Verify RFC 9238 headers (RateLimit-*) use delta seconds
    const resetDelta = parseInt(response.headers.get('RateLimit-Reset') || '0', 10);
    expect(resetDelta).toBeGreaterThan(0);   // Delta seconds must be positive
    expect(resetDelta).toBeLessThanOrEqual(60);  // Must be within window size

    // Verify Retry-After uses delta seconds (same as RateLimit-Reset)
    const retryAfter = parseInt(response.headers.get('Retry-After') || '0', 10);
    expect(retryAfter).toBe(resetDelta);  // Should match RateLimit-Reset

    // Verify remaining and limit headers are consistent

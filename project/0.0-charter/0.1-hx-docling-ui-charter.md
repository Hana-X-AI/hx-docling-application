...
**Version**: 0.7.0
**Status**: ✅ APPROVED - Team Feedback Incorporated
**Created**: 2025-12-11
**Last Updated**: 2025-12-11
**Author**: Agent Zero (Claude Opus 4.5)
**Reviewed By**: GitHub Copilot, Code Analysis Agent, 11-Specialist Team
**Deep Review**: See `reviews/charter-review-deep-dive.md` (23 findings - all addressed)
**Team Review**: See `reviews/team-reviews/00-combined-team-review.md` (6 critical, 2 major, 28 minor - all addressed)
**Target Executor**: Claude Code

---

## Review Response Summary (v0.7.0)

This version incorporates feedback from the 11-specialist team review:

| Category | Count | Status | Reference |
|----------|-------|--------|-----------|
| Critical (DB) | 6 | ✅ All Addressed | Trinity (PostgreSQL DBA) |
| Major (FE/QA) | 2 | ✅ All Addressed | Neo, Julia |
| Minor | 28 | ✅ All Addressed | All specialists |

**v0.7.0 Key Additions (Team Feedback):**
- Section 6.5.2: Server vs Client Component Boundaries (FE-M1)
- Section 6.5.3: next.config.ts Specification (FE-M2)
- Section 6.9: SSE Loading States (FE-M3)
- Section 7.3: Compound Database Indexes (ARC-01)
- Section 12.5.5: Database User Setup Procedure (DB-C3)
- Section 12.5.6: PostgreSQL Server Configuration (DB-C4)
- Section 12.5.7: Backup & Disaster Recovery Strategy (DB-C5)
- Section 12.5.8: Backup Monitoring & Alerting (DB-C6)
- Section 13.3: Quality Gate Enforcement (QA-M1)
- Section 16.5: Sprint 1.5 Sub-Task Breakdown (ORK-01)
- Section 16.6: Sprint Handoff Checklist (ORK-02)
- Updated DATABASE_URL with connection pool and SSL (DB-C1, DB-C2)

**v0.6.0 Key Additions (Code Analysis Agent):**
...
#### Server vs Client Component Boundaries (Addresses FE-M1)

All components must be explicitly designated as Server or Client Components:

| Component | Type | Directive | Rationale |
|-----------|------|-----------|-----------|
| **Layout Components** | | | |
| `Header.tsx` | Server | (default) | Static content, no interactivity |
| `Footer.tsx` | Server | (default) | Static content, version display |
| `Container.tsx` | Server | (default) | Layout wrapper only |
| **Input Components** | | | |
| `UploadZone.tsx` | Client | `'use client'` | Drag-drop events, file input |
| `UrlInput.tsx` | Client | `'use client'` | Controlled input, validation |
| `FilePreview.tsx` | Client | `'use client'` | Displays selected file state |
| `ProgressCard.tsx` | Client | `'use client'` | Real-time SSE updates |
| **Results Components** | | | |
| `ResultsViewer.tsx` | Client | `'use client'` | Tab switching, user interaction |
| `MarkdownView.tsx` | Server | (default) | Render-only, can stream |
| `HtmlView.tsx` | Server | (default) | Render-only, can stream |
| `JsonView.tsx` | Server | (default) | Render-only, can stream |
| `RawView.tsx` | Server | (default) | Render-only, can stream |
| `DownloadButton.tsx` | Client | `'use client'` | Click handler, blob creation |
| `Skeleton.tsx` | Server | (default) | CSS-only animation |
| **History Components** | | | |
| `HistoryView.tsx` | Server | (default) | Initial SSR, pagination via URL params |
| `JobRow.tsx` | Server | (default) | Static row rendering |
| `JobDetail.tsx` | Client | `'use client'` | Modal interaction, download |
| `Pagination.tsx` | Server | (default) | Link-based navigation |
| **Error Components** | | | |
| `ErrorBoundary.tsx` | Client | `'use client'` | Required for error boundaries |
| `ErrorDisplay.tsx` | Client | `'use client'` | Interactive recovery actions |
| `ErrorRecovery.tsx` | Client | `'use client'` | Retry button handlers |
...
**Bundle Size Impact:**
- Server Components: 0 KB client bundle (rendered on server)
- Client Components: Included in client bundle
- Target: < 100KB initial JS bundle (excluding React runtime)
...
#### next.config.ts Specification (Addresses FE-M2)
...
const config: NextConfig = {
  ...
  // No explicit turbo config needed unless customizing

  // Image optimization (if using next/image)
  images: {
    remotePatterns: [
      // Add patterns if URL preview thumbnails needed
  ...
  env: {
    NEXT_PUBLIC_APP_VERSION: process.env.NEXT_PUBLIC_APP_VERSION || '1.0.0-dev',
    // MCP base URL: origin + fixed /mcp path (no trailing slash)
    NEXT_PUBLIC_MCP_BASE_URL: `${process.env.DOCLING_MCP_ORIGIN || 'http://hx-docling-mcp-server.hx.dev.local:8000'}/mcp`,
  },

  // Strict mode for better error detection
  reactStrictMode: true,

  // Standalone output for Docker deployment (Phase 2)
  output: 'standalone',

  // Headers for security (modern best practices)
  async headers() {
    return [
      {
        source: '/api/:path*',
        headers: [
          ...
          // Content Security Policy - baseline restrictive policy
          {
            ...
            value: "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; connect-src 'self' http://hx-docling-mcp-server.hx.dev.local:8000; font-src 'self'; object-src 'none'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'",
          },
          ...
          // Note: X-XSS-Protection removed (obsolete, can cause issues in modern browsers)
        ],
      },
    ];
  },

  // Rewrites for MCP proxy (optional - avoids CORS)
  // Uses DOCLING_MCP_ORIGIN (scheme+host only) + fixed /mcp path to avoid double/missing path segments
  async rewrites() {
    const mcpOrigin = process.env.DOCLING_MCP_ORIGIN || 'http://hx-docling-mcp-server.hx.dev.local:8000';
    return [
      {
        source: '/mcp/:path*',
  ...
  // Redirects (none needed for Phase 1)
  async redirects() {
    return [];
  },
...
- `output: 'standalone'` - Required for Docker deployment in Phase 2
- `reactStrictMode: true` - Catches potential issues during development
- `rewrites` - Proxies MCP requests to avoid CORS issues
- `headers` - Security headers for API routes
...
### 6.9 SSE Loading States (Addresses FE-M3)

This section defines explicit UI states for loading, transitions, and reconnection during SSE streaming.

#### 6.9.1 Stage Transition States

| State | Trigger | UI Behavior | Duration |
|-------|---------|-------------|----------|
| **Initializing** | Form submitted | Spinner + "Preparing..." message | < 500ms |
| **Stage Transition** | Progress stage changes | Brief fade overlay + "Advancing..." | < 200ms |
| **Indeterminate** | No progress update for 5s | Pulsing animation + "Processing..." | Until update |
| **Near Complete** | Progress > 95% | "Finalizing..." message | Until 100% |

#### 6.9.2 Reconnection UI States

| State | Trigger | UI Display | User Actions |
|-------|---------|------------|--------------|
| **Connected** | Normal operation | Green status indicator | None |
| **Reconnecting** | Connection lost | Overlay: "Reconnecting..." + retry count (e.g., "Attempt 2/10") | Cancel button |
| **Fallback Polling** | SSE failed, polling active | Info banner: "Using backup connection" | None |
| **Failed** | All retries exhausted | Error modal with "Retry" and "Cancel" buttons | Retry / Cancel |

#### 6.9.3 Loading State Components

```typescript
// components/processing/LoadingStates.tsx

interface LoadingStateProps {
  state: 'initializing' | 'transitioning' | 'indeterminate' | 'reconnecting' | 'failed';
  retryCount?: number;
  maxRetries?: number;
  onRetry?: () => void;
  onCancel?: () => void;
...
const stateMessages: Record<string, string> = {
  initializing: 'Preparing document for processing...',
  transitioning: 'Advancing to next stage...',
  indeterminate: 'Processing... This may take a moment.',
  reconnecting: 'Connection interrupted. Reconnecting...',
  failed: 'Could not maintain connection to server.',
};

// Visual indicators
const stateIndicators = {
  initializing: <Spinner className="animate-spin" />,
  transitioning: <ArrowRight className="animate-pulse" />,
  indeterminate: <Loader className="animate-pulse" />,
  reconnecting: <RefreshCw className="animate-spin" />,
  failed: <AlertCircle className="text-destructive" />,
};
```

#### 6.9.4 Timeout Handling

| Scenario | Timeout | Action | User Feedback |
|----------|---------|--------|---------------|
| No initial response | 10s | Show warning | "Taking longer than expected..." |
| Stage stuck | 30s | Trigger health check | "Checking status..." |
| Total processing | 5min (configurable) | Offer cancel option | "Processing is taking a long time. Cancel?" |
| Network timeout | 30s | Start reconnection | "Reconnecting..." overlay |

#### 6.9.5 Progress Skeleton

During initial loading and reconnection, display a skeleton that matches the final progress UI:

```typescript
// components/processing/ProgressSkeleton.tsx
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex justify-between">
          <Skeleton className="h-4 w-24" /> {/* Stage name */}
          <Skeleton className="h-4 w-12" /> {/* Percentage */}
        </div>
        <Skeleton className="h-3 w-64" />   {/* Status message */}
      </CardContent>
    </Card>
  );
}
...
#### 6.9.6 Accessibility During Loading

| Requirement | Implementation |
|-------------|----------------|
| Screen reader announcements | `aria-live="polite"` for progress updates |
| Loading state announcement | `aria-busy="true"` on progress container |
| Reconnection announcement | `role="alert"` for reconnection status |
| Cancel keyboard access | `Escape` key triggers cancel during reconnection |
| Focus management | Focus trapped in modal during reconnection overlay |
...
model Job {
  ...
  // Compound indexes (Addresses ARC-01)
  // Optimized for common query patterns
  @@index([sessionId, status])     // Query: jobs by session filtered by status
  @@index([sessionId, createdAt])  // Query: paginated history for session
}
...
model Result {
  ...
  // Compound indexes (Addresses ARC-01)
  // Optimized for result lookup patterns
  @@index([jobId, format])         // Query: specific format for job (most common)
}
...
# MCP Server
# DOCLING_MCP_ORIGIN: scheme + host only (no path, no trailing slash)
# The /mcp path is appended automatically in next.config.ts to avoid path duplication
...
# Derived: NEXT_PUBLIC_MCP_BASE_URL = DOCLING_MCP_ORIGIN + '/mcp'
# (set automatically in next.config.ts, do not set manually)
...
# Connection pool: 5 connections per instance, 20s timeout
# SSL: Full verification with CA certificate (sslmode=verify-full)
# Certificate files stored in /etc/docling/ssl/ (chmod 600, owned by app user)
DATABASE_URL=postgresql://docling_user:${DB_PASSWORD}@hx-postgres-server.hx.dev.local:5432/docling_db?connection_limit=5&pool_timeout=20&sslmode=verify-full&sslrootcert=/etc/docling/ssl/ca.crt&sslcert=/etc/docling/ssl/client.crt&sslkey=/etc/docling/ssl/client.key

# Migration user (higher privileges for schema changes - use only in CI/CD)
# DATABASE_URL_MIGRATION=postgresql://docling_migration:${DB_MIGRATION_PASSWORD}@hx-postgres-server.hx.dev.local:5432/docling_db?sslmode=verify-full&sslrootcert=/etc/docling/ssl/ca.crt
...
# .env.test
DATABASE_URL=postgresql://docling_user:password@hx-postgres-server.hx.dev.local:5432/docling_db_test?connection_limit=5&pool_timeout=20&sslmode=require
...
#### Database User Setup (Addresses DB-C3)

```sql
-- Execute on hx-postgres-server (192.168.10.208) as postgres superuser
-- Create database
CREATE DATABASE docling_db;

-- Create user with password from Ansible Vault
CREATE USER docling_user WITH PASSWORD 'SECURE_PASSWORD_FROM_VAULT';

-- Grant minimal required privileges (least-privilege principle)
GRANT CONNECT ON DATABASE docling_db TO docling_user;
\c docling_db
GRANT USAGE ON SCHEMA public TO docling_user;
GRANT CREATE ON SCHEMA public TO docling_user;

-- Grant DML permissions on all current and future tables
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO docling_user;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO docling_user;

-- Grant sequence permissions for auto-increment (UUID doesn't need this, but for completeness)
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO docling_user;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT USAGE, SELECT ON SEQUENCES TO docling_user;
...
#### PostgreSQL Server Configuration (Addresses DB-C4)

**Target Server**: hx-postgres-server (192.168.10.208)

```ini
# postgresql.conf recommended settings for docling_db workload
# Location: /etc/postgresql/16/main/postgresql.conf

# Memory Configuration (assuming 32GB server RAM)
...
# Connection Settings
max_connections = 100                   # Sufficient for Next.js connection pooling
...
# Autovacuum (critical for preventing table bloat)
autovacuum = on
...
track_io_timing = on                    # For pg_stat_statements

# SSL Configuration (mandatory - full certificate verification)
ssl = on
ssl_cert_file = '/etc/ssl/certs/hx-postgres-server.crt'
ssl_key_file = '/etc/ssl/private/hx-postgres-server.key'
ssl_ca_file = '/etc/ssl/certs/hx-ca.crt'              # CA for client cert verification
```
...
# SSL with client certificate required for all connections from HX network
hostssl docling_db    docling_user      192.168.10.0/24    scram-sha-256    clientcert=verify-full
hostssl docling_db    docling_migration 192.168.10.0/24    scram-sha-256    clientcert=verify-full
hostssl docling_db    docling_user      127.0.0.1/32       scram-sha-256    clientcert=verify-full

# Reject non-SSL connections
hostnossl all         all               0.0.0.0/0          reject
```

#### SSL Certificate Storage (Client-Side)

**Location on hx-cc-server:** `/etc/docling/ssl/`

| File | Purpose | Permissions | Owner |
|------|---------|-------------|-------|
| `ca.crt` | CA certificate (hx-ca-server) | 644 | root:root |
| `client.crt` | Client certificate for docling_user | 600 | docling:docling |
| `client.key` | Client private key | 600 | docling:docling |

**Setup Commands:**
```bash
# Create secure directory
sudo mkdir -p /etc/docling/ssl
sudo chmod 700 /etc/docling/ssl

# Copy certificates from hx-ca-server (via secure channel)
sudo cp /path/to/hx-ca.crt /etc/docling/ssl/ca.crt
sudo cp /path/to/docling-client.crt /etc/docling/ssl/client.crt
sudo cp /path/to/docling-client.key /etc/docling/ssl/client.key

# Set ownership and permissions
sudo chown root:root /etc/docling/ssl/ca.crt
sudo chmod 644 /etc/docling/ssl/ca.crt
sudo chown docling:docling /etc/docling/ssl/client.crt /etc/docling/ssl/client.key
sudo chmod 600 /etc/docling/ssl/client.crt /etc/docling/ssl/client.key
```

#### Least-Privilege User Strategy

| User | Purpose | Privileges | Used By |
|------|---------|------------|---------|
| `docling_user` | Runtime application queries | SELECT, INSERT, UPDATE, DELETE on tables | Next.js application |
| `docling_migration` | Schema migrations only | ALL on schema + CREATE, ALTER, DROP | Prisma migrate (CI/CD only) |

**Migration User Setup:**
```sql
-- Create migration user with elevated privileges (CI/CD only)
CREATE USER docling_migration WITH PASSWORD 'secure_migration_password';
GRANT ALL PRIVILEGES ON DATABASE docling_db TO docling_migration;
GRANT ALL PRIVILEGES ON SCHEMA public TO docling_migration;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO docling_migration;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES TO docling_migration;

-- IMPORTANT: docling_migration credentials stored in CI/CD secrets only
-- Never expose in application .env files
```

#### Backup & Disaster Recovery Strategy (Addresses DB-C5)
...
|-------------|-----------|-----------|---------|
| Full pg_dump | Daily 01:00 UTC | 30 days | /backup/postgresql/daily/ |
| WAL Archive | Continuous | 7 days | /var/lib/postgresql/wal_archive/ |
| Weekly Full | Sunday 02:00 UTC | 90 days | /backup/postgresql/weekly/ |

**Daily Backup Script:**
```bash
#!/bin/bash
# /usr/local/bin/backup-docling-db.sh

set -euo pipefail

BACKUP_DIR="/backup/postgresql/daily"
DATE=$(date +%Y%m%d_%H%M%S)
BACKUP_FILE="${BACKUP_DIR}/docling_db_${DATE}.sql.gz"
...
pg_dump -h localhost -U postgres -d docling_db \
  --format=custom \
  --compress=9 \
  --file="${BACKUP_FILE}" \
  2>> "${LOG_FILE}"
...
if pg_restore --list "${BACKUP_FILE}" > /dev/null 2>&1; then
  echo "[$(date -Iseconds)] Backup verified: ${BACKUP_FILE}" >> "${LOG_FILE}"
else
  echo "[$(date -Iseconds)] ERROR: Backup verification failed!" >> "${LOG_FILE}"
  exit 1
fi

# Remove backups older than 30 days
find "${BACKUP_DIR}" -name "docling_db_*.sql.gz" -mtime +30 -delete

echo "[$(date -Iseconds)] Backup completed successfully" >> "${LOG_FILE}"
```
...
[Unit]
Description=Daily PostgreSQL Backup for docling_db
...
```ini
# /etc/systemd/system/docling-backup.service
[Unit]
Description=PostgreSQL Backup for docling_db
After=postgresql.service

[Service]
Type=oneshot
ExecStart=/usr/local/bin/backup-docling-db.sh
User=postgres
```
...
sudo -u postgres pg_basebackup -D /var/lib/postgresql/16/main -R

# 4. Create recovery.signal
sudo -u postgres touch /var/lib/postgresql/16/main/recovery.signal

# 5. Configure recovery target (in postgresql.conf)
# recovery_target_time = '2025-12-11 14:30:00 UTC'
...
#### Backup Monitoring & Alerting (Addresses DB-C6)

**Monitoring Checks:**

| Check | Frequency | Alert Threshold | Action |
|-------|-----------|-----------------|--------|
| Backup file exists | Hourly | No backup in 26 hours | Critical alert |
| Backup size | Daily | < 1KB (empty backup) | Critical alert |
| WAL archive lag | Every 5 min | > 100 segments | Warning alert |
| Backup verification | Daily | Verification fails | Critical alert |

**Monitoring Script:**
```bash
#!/bin/bash
# /usr/local/bin/check-docling-backup.sh

BACKUP_DIR="/backup/postgresql/daily"
MAX_AGE_HOURS=26
ALERT_EMAIL="ops-team@hx.dev.local"

# Check if recent backup exists
LATEST_BACKUP=$(find "${BACKUP_DIR}" -name "docling_db_*.sql.gz" -mmin -$((MAX_AGE_HOURS * 60)) | head -1)

if [ -z "${LATEST_BACKUP}" ]; then
  echo "CRITICAL: No backup found in last ${MAX_AGE_HOURS} hours" | \
    mail -s "[ALERT] docling_db backup missing" "${ALERT_EMAIL}"
  exit 2
fi
...
if [ "${BACKUP_SIZE}" -lt 1024 ]; then
  echo "CRITICAL: Backup file too small (${BACKUP_SIZE} bytes)" | \
    mail -s "[ALERT] docling_db backup corrupted" "${ALERT_EMAIL}"
  exit 2
fi

echo "OK: Backup healthy - ${LATEST_BACKUP} (${BACKUP_SIZE} bytes)"
exit 0
```
  ...
  - name: postgresql_backup
    rules:
      - alert: PostgreSQLBackupMissing
        expr: time() - postgresql_backup_last_success_timestamp > 93600
        for: 5m
        labels:
          severity: critical
        annotations:
          summary: "PostgreSQL backup missing for docling_db"
          description: "No successful backup in the last 26 hours"

      - alert: PostgreSQLWALArchiveLag
        expr: postgresql_wal_archive_pending_count > 100
        for: 10m
        labels:
          severity: warning
        annotations:
          summary: "PostgreSQL WAL archive lagging"
...
### 13.3 Quality Gate Enforcement (Addresses QA-M1)

This section defines **how** quality gates are enforced, not just what the thresholds are.

#### 13.3.1 Enforcement Toolchain

| Tool | Purpose | Configuration File |
|------|---------|-------------------|
| **husky** | Pre-commit hooks | `.husky/pre-commit` |
| **lint-staged** | Run linters on staged files only | `.lintstagedrc.json` |
| **GitHub Actions** | CI/CD pipeline gates | `.github/workflows/ci.yml` |
| **TypeScript** | Type checking | `tsconfig.json` |
| **ESLint** | Code quality rules | `eslint.config.mjs` |
| **Vitest** | Test runner & coverage | `vitest.config.ts` |

#### 13.3.2 Pre-Commit Hook Configuration
...
jobs:
  quality-gates:
    ...
    steps:
      ...
      # Gate 2: ESLint - 0 errors
      - name: Lint
        run: npm run lint

      # Gate 3: Unit Tests - 100% pass, 80% coverage
      - name: Test with Coverage
        run: npm run test:coverage

      # Gate 4: Build - must succeed
      - name: Build
        run: npm run build
  ...
  e2e-tests:
    ...
    steps:
      ...
      - name: Run E2E Tests
        ...
        env:
          DATABASE_URL: postgresql://test:test@localhost:5432/docling_test
          REDIS_URL: redis://localhost:6379
```

#### 13.3.4 Package.json Scripts
...
```json
{
  "scripts": {
    ...
    "test": "vitest",
    "test:coverage": "vitest run --coverage --coverage.thresholds.lines=80 --coverage.thresholds.branches=75",
...
#### 13.3.5 Gate Failure Behavior

| Gate | Local (pre-commit) | CI (PR) | CI (main) |
|------|-------------------|---------|-----------|
| TypeScript errors | ❌ Block commit | ❌ Block merge | ❌ Fail build |
| ESLint errors | ❌ Block commit | ❌ Block merge | ❌ Fail build |
| ESLint warnings > 10 | ⚠️ Warning only | ❌ Block merge | ⚠️ Warning only |
| Test failures | N/A (not in hook) | ❌ Block merge | ❌ Fail build |
| Coverage < 80% | N/A (not in hook) | ❌ Block merge | ❌ Fail build |
| Build failure | N/A (not in hook) | ❌ Block merge | ❌ Fail build |
| A11y violations | N/A (not in hook) | ⚠️ Warning only | ⚠️ Warning only |

#### 13.3.6 Branch Protection Rules

Configure on GitHub repository:

- **Required status checks**: `quality-gates`, `e2e-tests`
...
### 16.3 Team Assignments

> **Reference**: See `project/0.6-governance/0.6.5-team-roster.md` for detailed team responsibilities and RACI matrix.

| Sprint | Lead | Support | Review |
|--------|------|---------|--------|
| 1.1 | Neo (`@neo`) | William, Trinity | Alex |
| 1.2 | William (`@william`) | Trinity, Neo | Alex |
| 1.3 | Neo (`@neo`) | Ola, William | Julia |
| 1.4 | Neo (`@neo`) | Ola | Julia |
| 1.5 | James (`@james`) | Neo | Alex |
| **Checkpoint** | Julia (`@julia`) | Neo, William, Trinity | - |
| 1.6 | Neo (`@neo`) | Ola | Julia |
| 1.7 | Neo (`@neo`) | Ola, Trinity | Julia |
| 1.8 | Julia (`@julia`) | Neo, Trinity, William | Agent Zero |

### 16.4 Team Roster Summary

| Agent | Invocation | Primary Focus |
|-------|------------|---------------|
| Agent Zero | `@agent-zero` | Orchestration, quality gates |
| Alex Rivera | `@alex` | Architecture, ADRs |
| Julia Santos | `@julia` | Testing, QA |
| William Chen | `@william` | Infrastructure, operations |
| Trinity | `@trinity` | PostgreSQL DBA |
| Neo | `@neo` | Next.js, React, Tailwind (PRIMARY DEVELOPER) |
| Ola Mae Johnson | `@ola` | Frontend UI, accessibility |
| James Dean | `@james` | Docling MCP integration |
| Frank Lucas | `@frank` | DNS, SSL, certificates (Phase 2) |
| Thomas Anderson | `@thomas` | Docker, containers (Phase 2) |

### 16.5 Sprint 1.5 Sub-Task Breakdown (Addresses ORK-01)

Sprint 1.5 is the **most complex sprint** in Phase 1, integrating MCP client, SSE streaming, and error recovery. This breakdown ensures explicit handoff points and integration checkpoints.

#### 16.5.1 Task Decomposition

| Sub-Task | Lead | Duration | Deliverable | Handoff Criteria |
|----------|------|----------|-------------|------------------|
| **1.5a: MCP Client Library** | James (`@james`) | 1.5h | `lib/mcp/client.ts` | 8 tools callable, TypeScript types |
| **Handoff Point A** | - | 0.5h | Code review | Type check passes, no lint errors |
| **1.5b: SSE Manager** | William (`@william`) | 1.5h | `lib/sse/manager.ts`, `lib/sse/reconnect.ts` | 30s reconnect, polling fallback |
| **Handoff Point B** | - | 0.5h | Integration test | Unit tests pass (80%+ coverage) |
| **1.5c: API Route Integration** | Neo (`@neo`) | 1h | `/api/process/route.ts` | Route handles MCP + SSE |
| **Handoff Point C** | - | 0.5h | Code review | Route integration tested |
| **1.5d: UI Integration** | Ola (`@ola`) | 1h | `ProgressCard.tsx` integration | Progress UI updates correctly |
| **Integration Checkpoint** | Julia (`@julia`) | 1h | End-to-end test | Full flow passes |

**Total Sprint Duration**: ~7 hours (including handoffs)

#### 16.5.2 Dependency Graph

```mermaid
graph LR
    A[1.5a: MCP Client] --> HP1[Handoff A]
    HP1 --> B[1.5b: SSE Manager]
    B --> HP2[Handoff B]
    HP2 --> C[1.5c: API Route]
    C --> HP3[Handoff C]
    HP3 --> D[1.5d: UI Integration]
    D --> IC[Integration Checkpoint]
...
#### 16.5.3 Sub-Task Details

**1.5a: MCP Client Library** (James `@james`)

Deliverables:
- `lib/mcp/client.ts` - MCP client implementation
- `lib/mcp/types.ts` - TypeScript types for MCP protocol
- `lib/mcp/tools.ts` - Tool definitions for 8 Phase 1 tools

Acceptance:
- [ ] All 8 MCP tools callable: `convert_pdf`, `convert_docx`, `convert_xlsx`, `convert_pptx`, `convert_url`, `export_markdown`, `export_html`, `export_json`
- [ ] TypeScript types for request/response
- [ ] Error handling for MCP failures
- [ ] Unit tests with mocked MCP server

**1.5b: SSE Manager** (William `@william`)

Deliverables:
- `lib/sse/manager.ts` - SSE connection management
- `lib/sse/reconnect.ts` - Reconnection logic with backoff

Acceptance:
- [ ] Connection lifecycle: open, message, close, error
- [ ] Exponential backoff: 1s → 2s → 4s → 8s → 16s → 30s max
- [ ] Reconnection within 30s grace period
- [ ] Fallback to polling after SSE failure
- [ ] State synchronization on reconnect
- [ ] Unit tests for all reconnection scenarios

**1.5c: API Route Integration** (Neo `@neo`)

Deliverables:
- `/api/process/route.ts` - Processing endpoint with SSE

Acceptance:
- [ ] POST handler creates job in database
- [ ] SSE stream sends progress events
- [ ] Reconnection supported via `Last-Event-ID`
- [ ] Error events sent on failure
- [ ] Job state persisted for recovery
- [ ] Integration test with real database

**1.5d: UI Integration** (Ola `@ola`)

Deliverables:
- Updated `ProgressCard.tsx` with SSE integration
- Loading states from Section 6.9

Acceptance:
- [ ] Progress bar updates in real-time
- [ ] Stage names displayed correctly
- [ ] Reconnection overlay when needed
- [ ] Error display on failure
- [ ] Accessibility: screen reader announcements

#### 16.5.4 Integration Checkpoint Criteria

Before proceeding to Sprint 1.6, the following must pass:
...
### 16.6 Sprint Handoff Checklist (Addresses ORK-02)

Use this checklist at the end of every sprint before moving to the next:
...
- [ ] **Dependencies**
  ...
  - [ ] No outdated packages needing update

- [ ] **Integration**
  - [ ] All dependent services accessible (MCP, PostgreSQL, Redis)
...
Known Issues:
- Issue 1: [description, severity, owner]

Ready for Sprint 1.Y: ✅ YES / ❌ NO (reason)
...
### 19.1 Project Governance Documents

| Document | Location |
|----------|----------|
| Team Roster | `project/0.6-governance/0.6.5-team-roster.md` |
| RAIDD Log | `project/0.6-governance/0.6.1-raidd-log.md` |
| Status Report | `project/0.6-governance/0.6.2-status-report.md` |
| Backlog | `project/0.6-governance/0.6.3-backlog.md` |
| Lessons Learned | `project/0.6-governance/0.6.4-lessons-learned.md` |

### 19.2 HX-Infrastructure References
...
| 0.6.1 | 2025-12-11 | Agent Zero | **Team Assignment Update** |
| | | | - Added Section 16.3 Team Assignments |
| | | | - Added Section 16.4 Team Roster Summary |
| | | | - Added Section 19.1 Project Governance Documents |
| | | | - Neo (`@neo`) designated as PRIMARY DEVELOPER |
| | | | - Trinity (`@trinity`) designated as PostgreSQL DBA |
| | | | - Aligned with Team Roster v2.1.0 |
| 0.7.0 | 2025-12-11 | Agent Zero | **11-Specialist Team Feedback Integration** |
| | | | **Critical Database Gaps (Trinity - DBA):** |
| | | | - DB-C1: Added connection pool config to DATABASE_URL |
| | | | - DB-C2: Added SSL/TLS requirement (sslmode=require) |
| | | | - DB-C3: Added Section 12.5.5 Database User Setup |
| | | | - DB-C4: Added Section 12.5.6 PostgreSQL Server Config |
| | | | - DB-C5: Added Section 12.5.7 Backup & Disaster Recovery |
| | | | - DB-C6: Added Section 12.5.8 Backup Monitoring & Alerting |
| | | | **Major Frontend/QA Gaps (Neo, Julia):** |
| | | | - FE-M1: Added Section 6.5.2 Server vs Client Components |
| | | | - FE-M2: Added Section 6.5.3 next.config.ts Specification |
| | | | - FE-M3: Added Section 6.9 SSE Loading States |
| | | | - QA-M1: Added Section 13.3 Quality Gate Enforcement |
| | | | **Minor Gaps Addressed:** |
| | | | - ARC-01: Added compound database indexes (Section 7.3) |
| | | | - ORK-01: Added Section 16.5 Sprint 1.5 Sub-Task Breakdown |
| | | | - ORK-02: Added Section 16.6 Sprint Handoff Checklist |
| | | | **Review Reference:** |
| | | | - `reviews/team-reviews/00-combined-team-review.md` |
| | | | - `reviews/team-reviews/0.0.1-key-gaps.md` |
...
*All 23 findings from Code Analysis Agent review and all 36 findings from 11-specialist team review have been addressed.*